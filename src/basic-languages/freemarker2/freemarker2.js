...
exports.TagAutoInterpolationBracket =
	exports.TagAutoInterpolationDollar =
	exports.TagBracketInterpolationBracket =
	exports.TagAngleInterpolationBracket =
	exports.TagBracketInterpolationDollar =
	exports.TagAngleInterpolationDollar =
		void 0;
var monaco_editor_core_1 = require('../../fillers/monaco-editor-core');
		...
		folding: {
			markers: {
				start: new RegExp(
					''
						.concat(ts.open, '#(?:')
						.concat(BLOCK_ELEMENTS.join('|'), ')([^/')
						.concat(ts.close, ']*(?!/)')
						.concat(ts.close, ')[^')
						.concat(ts.open, ']*$')
				),
				end: new RegExp(
					''.concat(ts.open, '/#(?:').concat(BLOCK_ELEMENTS.join('|'), ')[\\r\\n\\t ]*>')
				)
			}
		},
		onEnterRules: [
			{
				beforeText: new RegExp(
					''
						.concat(ts.open, '#(?!(?:')
						.concat(EMPTY_ELEMENTS.join('|'), '))([a-zA-Z_]+)([^/')
						.concat(ts.close, ']*(?!/)')
						.concat(ts.close, ')[^')
						.concat(ts.open, ']*$')
				),
				afterText: new RegExp(
					'^'.concat(ts.open, '/#([a-zA-Z_]+)[\\r\\n\\t ]*').concat(ts.close, '$')
				),
				action: {
...
function createMonarchLanguage(ts, is) {
	...
	return (
		...
		// Dynamic RegExp
		(_a[s('open__id__')] = new RegExp(ts.open)),
		(_a[s('close__id__')] = new RegExp(ts.close)),
		(_a[s('iOpen1__id__')] = new RegExp(is.open1)),
		(_a[s('iOpen2__id__')] = new RegExp(is.open2)),
		(_a[s('iClose__id__')] = new RegExp(is.close)),
		// <#START_TAG : "<" | "<#" | "[#">
		// <#END_TAG : "</" | "</#" | "[/#">
		(_a[s('startTag__id__')] = r(/(@open__id__)(#)/)),
		(_a[s('endTag__id__')] = r(/(@open__id__)(\/#)/)),
		(_a[s('startOrEndTag__id__')] = r(/(@open__id__)(\/?#)/)),
		// <#CLOSE_TAG1 : (<BLANK>)* (">" | "]")>
		(_a[s('closeTag1__id__')] = r(/((?:@blank)*)(@close__id__)/)),
		// <#CLOSE_TAG2 : (<BLANK>)* ("/")? (">" | "]")>
		(_a[s('closeTag2__id__')] = r(/((?:@blank)*\/?)(@close__id__)/)),
		// Static RegExp
		// <#BLANK : " " | "\t" | "\n" | "\r">
		(_a.blank = /[ \t\n\r]/),
		// <FALSE : "false">
		...
		(_a.escapedChar = /\\(?:[ntrfbgla\\'"\{=]|(?:x[0-9A-Fa-f]{1,4}))/),
		// <#ASCII_DIGIT: ["0" - "9"]>
		(_a.asciiDigit = /[0-9]/),
		// <INTEGER : (["0"-"9"])+>
		(_a.integer = /[0-9]+/),
		...
		(_a.idStartChar = /(?:@nonEscapedIdStartChar)|(?:@escapedIdChar)/),
		// <ID: <ID_START_CHAR> (<ID_START_CHAR>|<ASCII_DIGIT>)*>
		(_a.id = /(?:@idStartChar)(?:(?:@idStartChar)|(?:@asciiDigit))*/),
		...
		// 		}
		// }
		(_a.specialHashKeys = /\*\*|\*|false|true|in|as|using/),
		// <DOUBLE_EQUALS : "==">
		// <EQUALS : "=">
		// <NOT_EQUALS : "!=">
		// <PLUS_EQUALS : "+=">
		...
		(_a.arrows = ['->', '-&gt;']),
		(_a.delimiters = [';', ':', ',', '.']),
		(_a.stringOperators = ['lte', 'lt', 'gte', 'gt']),
		(_a.noParseTags = ['noparse', 'noParse', 'comment']),
		(_a.tokenizer =
			((_b = {}),
			// Parser states
			// Plain text
			...
			// <@join [1,2] ",">
			// <@null!join [1,2] ",">
			(_b[s('unifiedCall__id__')] = [{ include: s('@unified_call_token__id__') }]),
			// For singly and doubly quoted string (that may contain interpolations)
			(_b[s('singleString__id__')] = [{ include: s('@string_single_token__id__') }]),
			(_b[s('doubleString__id__')] = [{ include: s('@string_double_token__id__') }]),
			// For singly and doubly quoted string (that may not contain interpolations)
			(_b[s('rawSingleString__id__')] = [{ include: s('@string_single_raw_token__id__') }]),
			(_b[s('rawDoubleString__id__')] = [{ include: s('@string_double_raw_token__id__') }]),
			// For a comment in an expression
			// ${ 1 + <#-- comment --> 2}
			(_b[s('expressionComment__id__')] = [{ include: s('@expression_comment_token__id__') }]),
			// For <#noparse> ... </#noparse>
			// For <#noParse> ... </#noParse>
			// For <#comment> ... </#comment>
			(_b[s('noParse__id__')] = [{ include: s('@no_parse_token__id__') }]),
			// For <#-- ... -->
			(_b[s('terseComment__id__')] = [{ include: s('@terse_comment_token__id__') }]),
			// Common rules
			(_b[s('directive_token__id__')] = [
				...
				[
					...
					ts.id === 'auto'
						? {
								cases: {
									'$1==<': { token: '@rematch', switchTo: '@default_angle_'.concat(is.id) },
									'$1==[': { token: '@rematch', switchTo: '@default_bracket_'.concat(is.id) }
								}
						  }
						: [
			...
			(_b[s('interpolation_and_text_token__id__')] = [
			...
			// 	)
			// >
			(_b[s('string_single_token__id__')] = [
				[/[^'\\]/, { token: 'string' }],
				[/@escapedChar/, { token: 'string.escape' }],
				[/'/, { token: 'string', next: '@pop' }]
			]),
			(_b[s('string_double_token__id__')] = [
				[/[^"\\]/, { token: 'string' }],
				[/@escapedChar/, { token: 'string.escape' }],
				[/"/, { token: 'string', next: '@pop' }]
			]),
			// <RAW_STRING : "r" (("\"" (~["\""])* "\"") | ("'" (~["'"])* "'"))>
			(_b[s('string_single_raw_token__id__')] = [
				[/[^']+/, { token: 'string.raw' }],
				[/'/, { token: 'string.raw', next: '@pop' }]
			]),
			(_b[s('string_double_raw_token__id__')] = [
				[/[^"]+/, { token: 'string.raw' }],
				[/"/, { token: 'string.raw', next: '@pop' }]
			]),
			// <FM_EXPRESSION, IN_PAREN, NO_SPACE_EXPRESSION, NAMED_PARAMETER_EXPRESSION> TOKEN :
			(_b[s('expression_token__id__')] = [
				// Strings
				[
					/(r?)(['"])/,
					{
						...
						cases: {
							...
							'\\]': {
								cases: __assign(
									...
									((_c = {}),
									(_c[s('$S1==inParen__id__')] = { token: '@brackets', next: '@pop' }),
									(_c['@default'] = { token: '@brackets' }),
									_c)
								)
							},
							...
							'\\)': {
								cases:
									((_d = {}),
									(_d[s('$S1==inParen__id__')] = { token: '@brackets', next: '@pop' }),
									(_d['@default'] = { token: '@brackets' }),
									_d)
							},
							'\\{': {
								cases: {
							...
							'\\}': {
								cases: __assign(
									...
									((_e = {}),
									(_e[s('$S1==inParen__id__')] = { token: '@brackets', next: '@pop' }),
									(_e['@default'] = { token: '@brackets' }),
									_e)
								)
							}
						}
			...
			(_b[s('blank_and_expression_comment_token__id__')] = [
				// < ( " " | "\t" | "\n" | "\r" )+ >
				[/(?:@blank)+/, { token: '' }],
				// < ("<" | "[") ("#" | "!") "--"> : EXPRESSION_COMMENT
			...
			(_b[s('directive_end_token__id__')] = [
				// <DIRECTIVE_END : ">">
				// {
				//     if (inFTLHeader) {
				//         eatNewline();
			...
			// <IN_PAREN> TOKEN :
			(_b[s('greater_operators_token__id__')] = [
				// <NATURAL_GT : ">">
				[/>/, { token: 'operators' }],
				// <NATURAL_GTE : ">=">
				[/>=/, { token: 'operators' }]
			...
			(_b[s('no_space_expression_end_token__id__')] = [
				// <TERMINATING_WHITESPACE :  (["\n", "\r", "\t", " "])+> : FM_EXPRESSION
				[/(?:@blank)+/, { token: '', switchTo: s('@fmExpression__id__.directive') }]
			]),
			(_b[s('unified_call_token__id__')] = [
			...
			(_b[s('expression_comment_token__id__')] = [
				// < "-->" | "--]">
				[
					/--[>\]]/,
					{
			...
			(_b[s('terse_comment_token__id__')] = [
				//  <TERSE_COMMENT_END : "-->" | "--]">
...
function createMonarchLanguageAuto(is) {
	...
	return __assign(__assign(__assign(__assign({}, angle), bracket), auto), {
		// Settings
		unicode: true,
		includeLF: false,
		...
		tokenizer: __assign(__assign(__assign({}, angle.tokenizer), bracket.tokenizer), auto.tokenizer)
	});
}
exports.TagAngleInterpolationDollar = {
